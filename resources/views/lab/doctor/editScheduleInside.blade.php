@extends('layouts.lab')
@section('title',trans('global.edit') .' '. trans('cruds.doctor.updateDoctorScheduleInside'))
@section('content')
        ...
        <div class="container">
            ...
            <div class="card card-custom example example-compact mt-4">
                <div class="card-header">
                    <h3 class="card-title">  {{ trans('cruds.doctor.updateDoctorScheduleInside') }}</h3>
                                ...
                                <div class="col-md-3 col-lg-3 fv-row">
                                    ...
                                    <div class="d-flex flex-column mb-8 fv-row">
                                        <select class="form-select form-select-solid" data-control="select2"
                                            ...
                                            <option value="1"
                                                    @if($object['active']) selected @endif
                                            > {{trans('global.active')}}</option>
                                            <option value="0"
                                                    @if(!$object['active']) selected @endif
                                    ...
                                    <select class="form-select form-select-solid" data-control="select2" multiple
                                        ...
                                        @foreach(staticHorses() as $staticHorses)
                                            <option value="{{$staticHorses}}"
                                                    @if(in_array($staticHorses , $object['work_hours']) ) selected @endif
    ...
    <script>
        ...
        $(document).on('click', '#btn_update_schedule', function (e) {
            ...
            $.ajax({
                type: 'post',
                enctype: 'multipart/form-data',
                url: "{{route("lab.doctor.update.scheduleInside",$doctor->id)}}",
